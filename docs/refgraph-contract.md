...
### `reference`（2026-08-06 追記・C-1）

すべての辺は `reference`（トークンの**原文・切り詰めない**・既定値 `""`）を持つ。`to_dict` は常に出力する。
`target` が解決や刻み（T-5 / T-5b）で書き換わっても、`reference` は元の文字列のまま残す（情報を消さない）。
...
- **刻む前に正規化しない。正規化（末尾 `/` の除去・`.` / `..` の畳み込み）は各前置詞を 4 段へ入れる直前に
  適用する**
- **`reference` は常に原文 run であり、`target` だけが刻まれた形になりうる**（両者の不一致は仕様であって
  実装バグではない）
...
- **`settled_links` のグループキーは `(relation, source, reference)`。** 畳んだ結果 1 候補になったグループの
  辺を返す
- **`glob_matches` の `*` は `[^/]*`、`**` は `[^/]*[^/]*` と等価であり、いずれも 1 成分内にしか一致しない。
  再帰的な階層展開は行わない。** この意味論は、抽出器の出力に中間 `**` を含む target（`src/**/*.py` 等）が
  現れるようになっても変わらない（C-22）
