...
- **10 のスキル**（`/init-session` / `/setup` / `/start` / `/develop` / `/review-phase` / `/promote-pattern` / `/doc` / `/mcp-config` / `/extract-lib` / `/recall`）
- **5 フェーズの開発ワークフロー**（ヒアリング → 設計 → 計画 → TDD → レビュー）
...
- **意味検索 recall (v2.10.0+)**: `.claude/memory/sessions/`・`.claude/agent-memory/`・`.claude/reports/archive/`・`.claude/memory/patterns.json` から類似情報を HNSW + 多言語 embedding (`paraphrase-multilingual-MiniLM-L12-v2`) で意味検索。`UserPromptSubmit` hook で親 Claude のコンテキストに「現タスクと無関係なら無視」前置き付きで自動注入

## 次に読むページ

- [はじめに](getting-started.md) — インストールから最初のセッションまで
- [スキル一覧](skills.md) — 10 のスキルの概要
