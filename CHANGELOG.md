...
- `.claude/CLAUDE.md` に「根本解決の原則（監査・レビューを通すための対応をしない）」節を追加した。
  監査（C-3）・レビュー（E）・テストの指摘に対し、指摘を消すためだけの対応（言い換え・周辺の付け足し・
  機構の差し替え）を禁じ、根本問題を 1 文で書いてそれを解く手段を採る原則 5 項目と、判断の手順 5 段階
  （根本問題の 1 文化 → 解く/隠すの仕分け → 修正範囲が大きくても設計・計画に載せる → 一次資料と再現実験で
  裏取り → 根本問題が解けたことを実測で示す）を定めた。対症療法しか取れない場合は理由を明記し、
  根本解決へ戻るトリガーをセッションファイルの残タスクに残す。是正ループの打切条件（3 連続で前提を疑う）は
  dev-workflow E-3 を唯一の正本とし、本節には重複記述を置かない。利用先プロジェクトで運用して不足と
  判明した規約を C3 本体へ取り込んだもの

### 変更

- `parallel-agents` skill の `model:` 指定規則と `docs/spec/02-hooks.md` の tier_autoapply 仕様表に、
  Claude Code v2.1.257 で追加された環境変数 `CLAUDE_CODE_SUBAGENT_MODEL_FORCE` の注意を追記した。
  同変数が設定されていると per-spawn の `model:`（hook 注入・明示指定とも）と agent 定義の `model:` が
  ハーネス側で無視され、tier_autoapply が記録した tier と実適用モデルが乖離する。C3 は同変数を設定しない
  （`CLAUDE_CODE_SUBAGENT_MODEL` は v2.1.251 以降既定値扱いで注入が優先されるため影響なし）
...
- `/CLAUDE.md` §2 と `.claude/docs/config-policy.md` §6 に、上記の機械検証 2 本と
  許容リスト・件数定数群の同時更新の運用を追記した（**配布元 C3 のリポジトリ専用の検査**である旨の
  但し書き付き。配布元 C3 が公開する wheel に `tests/` は含まれず利用先プロジェクトには配置されないが、
  配布元 C3 が PyPI へ公開する第二の成果物である sdist には `tests/` が含まれる。sdist は PyPI で
  公開されるため `tests/` 配下の内容は第三者が閲覧可能であり、機密情報を書いてはならない）。
...
- **E-0（実行検証判定）に「省略の出口（HITL 専用）」を追加した**。E-0 が NEEDS_VERIFY を
  返しても、当該ワークフロー内の test-report に検出対象への実行検証の実績があると
  **実物の Read で確認できた場合に限り**、証跡（test-report のファイル名・実績が記録されて
  いる節〔節名は問わない〕・検証内容の要旨・E-0 判定の件数）を提示して AskUserQuestion
  （「省略する」「実行検証を行う」の 2 択）で確認できる。省略可否を決めるのはユーザーであり、
  LLM の役割は実績確認・証跡提示・回答記録の 3 点に限る（機械的な適格判定・鮮度判定は
  持たない）。従来はこの状況（修正タスク自身が実行検証を内包しているのに E-0 が重複要求する。
  実発生 2 回）を定義されていないアドホック裁定で回避していた
- 省略成立時は E-0 判定行に `省略(既検証: {ファイル名} ユーザー承認済)` の証跡が残る
  （基本書式は保持・末尾追記のみ。拡張書式は HITL 専用で自律モードは基本書式のまま）

### 変更なし（明示）

- **既定挙動は従来どおり**（破壊的変更なし）。実績が確認できない・判断に迷う場合は質問を
  出さず従来どおり tester を起動する（fail-safe）。UNKNOWN は対象外。**自律モードでは本出口を
  適用しない**（autonomous-mode の「例外なく tester を起動する」規定は不変更）

### 内部

- `tests/skills/test_e0_verification_exit.py` を新規追加（P1〜P8 の文言凍結。行頭アンカーに
  よる節範囲切り出し・UNKNOWN 除外の負の対照・autonomous-mode 側文言の回帰ガードを含む）
- 設計過程の記録: 当初の自動省略設計（機械的文書条件による LLM 自動判定）は C-3 監査
  3 サイクル連続で新造欠陥を生み（DC 計 48 件）打切・1 問ゲートへ前提転換した
  （architecture-report ADR-8）

## [2.69.0] - 2026-08-13

### 修正

- **`tier_autoapply.py` のロック機構を非ブロック試行＋ms リトライへ置換した**（フレーキー根治・
  学習記録の行消失の構造的排除）。従来の `msvcrt.locking(LK_LOCK)` は CRT 仕様で「1 秒間隔
  リトライ・10 回で OSError」のため、複数 agent の同時起動時に待機側のリトライが 1 秒境界に
  同期するロックコンボイが発生し、リトライを枯渇した個体が無ロック追記フォールバックに落ちて
  **他プロセスの追記行を上書き消失させる**実欠陥があった（`TestConcurrency` の非決定的失敗
  〔2026-08-04 起票〕の根本原因。実運用の parallel-agents wave 一斉起動でも発生しうる）。
  新実装は LK_NBLCK / `LOCK_EX|LOCK_NB` の非ブロック試行＋ms 粒度リトライ（20ms 間隔＋
  0〜20ms ジッターで脱同期・`time.monotonic()` 基準の締切 5 秒）で、実測でコンボイ・行消失とも
  消滅（42 回反復失敗 0・20 並列時の最大取得待ち 0.109 秒）

### 変更

- **ロック取得失敗時の無ロック追記フォールバックを全廃し断念設計へ変更した**。ロック競合が
  締切 5 秒を超えた場合、当該 1 行の記録を断念して stderr に固定文言を出す（学習記録 1 行の
  欠落に留まり、他プロセスの行を壊さない）。記録処理到達後の断念 4 経路
  （`reason=deadline` / `reason=lockfile-open-failed` / `reason=mkdir-failed` /
  `reason=write-failed`）すべてが固定文言の stderr 証跡を持つ（沈黙断念ゼロ・外部由来文字列は
  含めない）。記録の断念は tier 機械適用（model 注入）に影響しない
- hook の最悪ブロック時間は従来の約 10 秒から締切 5 秒へ半減した

### 内部

- `tests/hooks/test_tier_autoapply.py` に P1〜P5 の性質テスト群・断念 4 経路の完全一致固定・
  実競合下のジッター検証（jitter=0 対照で赤化する検知力を実証済み）・ADR-5 / mkdir / write の
  回帰網を追加（52→56 テスト）。`TestConcurrency` は判定コード不変
- `docs/spec/00-index.md` の凍結宣言に注記 1 行を追加（02-hooks.md / 05-state-db.md の
  ロック記述は旧実装前提である旨）
...
- **フェーズ E に E-3（統合裁定）を新設した**（配布物・`dev-workflow/SKILL.md`）。
  code-review と security-review の**両レポートが揃ってから 1 回だけ**対応方針を裁定する。
  従来は E-1 承認 → E-2 承認の逐次 2 ゲートで、「E-1 で差し戻すと E-2 が次周回送りになる」ことを
  理由に指摘の重大性ではなく実行順序で裁定が動くバイアスがあった（2026-08-04 の整理・
  2026-08-09 の先行実測で差し戻し回数の削減を確認済み）。承認ゲートは 10 個 → **9 個**
  - E-1 / E-2 は「reviewer 起動＋hint 注入」だけの実行ステップとして存続（承認質問を持たない）
  - tier-routing の帰属 failure 記録は指摘の**出所別**（CR 由来= `--gate E-1`・SR 由来= `--gate E-2`）で
    従来互換。findings が無い出所のゲートへは failure を記録しない。完了時 success は
    E-1+E-2 各 1 件で `--final` は E-2 側のみ（記録件数・削除トリガの一意性を保存）
  - **「裁定の判断基準」4 項目を E-3 に明文化**（C-3 の design-critic findings 裁定からも参照）:
    (1) 是正案は最小パッチ / 構造的是正の 2 案比較を既定とする (2) 保守性指摘の許容には
    将来の運用コスト（PR diff 量・触る箇所数）見積りを必須で含める (3) 先送りはトリガー付き起票のみ
    (4) 共通化・機構追加はその機構自体の保守コストも同じ天秤に載せる
- **`references/record-protocol.md` を新設した**（配布物）。tier-routing 記録の説明規約
  （引数規約・機械適用・opus 固定不変則・集計注記・帰属根拠トークンの判定規約）を外出しし、
  各ゲートには record コマンド実体と gate 固有引数のみを残した（コマンドは現地・説明は reference）
- **`references/c3-omission-declaration.md` を新設した**（配布物）。C-3省略宣言の生成規約
  （提示条件・省略 4 条件・固定書式・転記規定・4 択 JSON の使い分け）を外出しし、
  C-2 本文は発火条件と導線へ縮減した。毎回の C-3 到達で効く fail-safe 群
  （判定対象の限定・複数マッチ不成立・タイムスタンプ照合・当日ファイル限定・消費処理）は
  SKILL.md 本文に残した（判定コアは本文・稀路の詳細は reference の分割基準）
- **C-3 ステップ 0 に転記行判定の適用順を明文化した**: (1) 判定対象の抽出 →
  (2) 複数マッチ検査（同一種別・異種混在を問わず現行 plan タイムスタンプ一致の未消費行が
  合計 2 行以上で「転記行なし」）→ (3) 優先順位表。監査要求と省略宣言が並存しても不成立側に倒れる
- 静的検査 `tests/skills/test_c3_omission_declaration.py` を新設した（22 件・非配布）。
  references 2 ファイルの必須要素・SKILL.md の導線・fail-safe 本文残置の番人を凍結する。
  `tests/skills/test_reachability_map.py` P2(b) は E-3 セクションの reachability-map 参照検査へ改訂

### 変更

- `dev-workflow/SKILL.md` は 1,251 行 → **1,106 行**（E-1/E-2 の裁定手順重複＝帰属判定
  ブロック 4 回反復の解消と references 外出しによる縮減）
- **C-3省略宣言の条件 2（帰属アンカー）は AND 判定を維持**したまま、E-3 統合裁定では
  CR/SR 両出所の failure 記録が通常となる旨を主文に明記した（省略宣言は事実上成立しにくくなる。
  fail-safe 方向の随伴効果として受容裁定済み・prompt-history 追記の高頻度化も同様）
- E-3 完了へ至る両分岐から「引き継ぎバックログの照合」共通ステップを参照する形へ整理し、
  バックログ照合の除外列挙へ `レビュー裁定` を追加した
- `autonomous-mode/SKILL.md`（配布物）を新構成へ同期した: ゲート対応表の E-1/E-2 の 2 行を
  E-3 の 1 行へ統合（自律動作は等価: CR∥SR 起動のみ並列・「全て対応する」固定・High/Medium 0 まで
  上限 5 周）・承認ゲート数 9 個へ・cycles 集約 ID `E` の定義を「E-1+E-2+E-3 を 1 セット」で統一・
  findings が無い出所のゲートへ failure を記録しない除外を join 手順にも明記
- `docs/spec/` は凍結裁定（2026-08-05）を優先して内容更新せず、凍結宣言に
  「S2 で E フェーズ構成が変更済み・本文書群の E 関連記述は旧構成」の注記 1 行のみ追加した
...
- `c3 update` の削除処理で、セーフガードが弾いた理由（warnings）や不在ファイル一覧が
  画面に出ないケースがあったのを修正した。従来は「削除を実行した」場合にしか詳細を出しておらず、
  (a) 削除候補が 0 件のとき (b) 削除プロンプトで n を選んだとき の 2 状態で握り潰されていた。
  影響は init-only 保護に限らず `deletions.txt` のパース警告を含むセーフガード全体に及んでいた
- 真因（詳細の出力コードが dry-run 経路と通常実行経路に二重化し、通常実行側では状態分岐に
  埋もれていた）を除去し、詳細の出力を単一・無条件の経路へ統合した
- 詳細行の本文に **C0 制御文字・ESC の除去**を適用した（表示する状態が増えたことに対する予防）。
  **bidi 制御文字（U+202E 等）は除去対象外**（既知・許容）

### 変更

- 削除実行時の詳細セクションの見出し書式が `--dry-run` 側に統一された。
  `  absent:` / `  warnings:` の入れ子見出しは `deletions: N already absent:` /
  `deletions: N warning(s):` に変わり、一覧のインデントが 4 スペースから 2 スペースになる
- `deletions: nothing to delete` は削除・不在・エラー・警告のいずれも無く、
  かつキャンセルされていないときにのみ表示する
...
- **`SessionStart:compact` hook の stdout がハーネスの上限を超え、コンパクト後のセッション復元が警告なく骨抜きになっていた**: ハーネスは hook の `additionalContext` / `systemMessage` / plain stdout を **10,000 文字**（バイトではない）で cap し、超過分はファイル保存＋プレビュー差し替えに置き換える（公式ドキュメント https://code.claude.com/docs/en/hooks で確認・**設定変更は不可**）。`SessionStart` はこの上限が適用されつつ stdout がそのまま context 化される 3 例外イベントの 1 つである。`.claude/hooks/restore_session.py` は出力 3 セクションのうち「うまくいったアプローチ」「試みたが失敗したアプローチ」にのみ `_tail(…, APPROACH_TAIL_LINES=15)` の行数上限を持ち、**残タスクだけが件数・長さとも無制限**だった。実測で 10,266 文字（うまくいった／失敗したアプローチが**空**の状態で）と上限を超過しており、切り詰めは「成功」扱いで警告されないため、**LLM が気付かなければ復元されたつもりで残タスクの大半と失敗パターンを失ったまま作業を再開する**状態だった
- **`現在地:` の値に長さ上限が無く、fail-loud マーカーを 10,000 文字境界の外へ押し出せた**: `現在地:` を埋め込むワークフロー復帰指示は出力の**先頭側**（マーカーより前方）に置かれるため、値が肥大するとマーカー自体が境界外へ出て、**ハーネスの truncate でマーカーごと静かに消える**。security-review が実機で再現し `MARKER_IDX = 15167`（境界外）を実測した。残タスク（文字数予算）・アプローチ 2 セクション（行数上限）はいずれもキャップを持つのに、**`現在地:` だけがどのキャップにも入っていない**という非対称が原因だった。これは仮説ではなく、本リリースの開発中に実セッションファイルの `現在地:` が 1,524 文字まで肥大した実インシデントとして発生している

### 追加

- **残タスクセクションの文字数予算と fail-loud マーカー**: 出力全体が 10,000 文字に収まるよう、固定部（復帰指示・ヘッダ・アプローチ 2 セクション）の実長を先に測って残りを残タスクへ配分する。切り詰めが発生した場合は**必ず**「全 N 件のうち先頭 M 件のみ表示」＋**セッションファイルの絶対パス**を出力し、LLM が「復元されたつもり」にならないようにする。マーカーは `## 残タスク` 見出しの**直後・本文より前**に置く（末尾に置くと、万一ハーネス側 truncate が発火したときマーカー自体が溢れて消え、元の問題がそのまま再現するため）。切り詰めが発生しない場合はマーカーを出さない（過検知しない）
- **`現在地:` 値のキャップ（`MAX_GENBA_CHARS = 200`）と fail-loud 表示**: 超過分は先頭 200 文字へ切り詰め、末尾に「全 N 文字中 先頭 200 文字のみ表示。全文は `{path}` を Read」を付ける。**これによりマーカーの生存が無条件保証になる**。マーカー位置の上界は「約 415 + パス長」で OS のパス長上限に抑えられるため（Windows 260 / Linux 4096）、残タスクやアプローチ 2 セクションがどれだけ肥大してもマーカーは 10,000 の内側に留まる。実測で `MARKER_IDX = 15167` → **505**（現在地 15,000 文字＋アプローチ 15 万文字超の複合ケースでも境界内）。「総長が 10,000 を超え得る」ことと「マーカーが消える」ことを別問題として切り離し、後者だけを完全に閉じた設計である
- **アプローチ 2 セクションの切り詰め通知**: 従来 `_tail` は 16 行目以降を**無言で**捨てていた。実際に切り詰めたときだけ見出しへ `（末尾 15 行のみ・全 N 行）` を付ける（`_tail` 本体のロジックは不変）。「切り詰めたら必ず知らせる」という原則を 3 セクションすべてに揃えた
- **`現在地:` の運用規約を明文化**: `.claude/skills/dev-workflow/SKILL.md` の運用総則に、`現在地:` へ書くのは「今どのフェーズか」「次にどこへ向かうか」の 2 点だけとし **200 文字以内**に収めること、経緯・確定事項・コミットハッシュ・再開手順は `## 残タスク` / `## うまくいったアプローチ` / `## 試みたが失敗したアプローチ` へ振り分けることを追記した。**「なぜ 200 文字なのか」も併記**している（上限だけを書くと美観の規則に見え、「引き継ぎのためだから例外」という自己正当化の余地が残るため）。あわせて `.claude/skills/init-session/SKILL.md` には、**この長さ規約を機械的に強制する仕組みは無い**こと（`MAX_GENBA_CHARS` / `_cap_genba` は `restore_session.py` が stdout を組み立てるときだけ働き、session.tmp を直接 Read する経路には関与しない。書き込み時点の長さ検証 hook も存在しない）を明記し、規約に反する長大な値が実在し得る前提で読むよう注記した

### 変更

- **`restore_session.py` の stdout を `newline='\n'` で reconfigure**: `TextIOWrapper.reconfigure` は `newline` 未指定だと既存設定（変換あり）を保つため、Windows では `print` の `\n` が `\r\n` になる。ハーネスは**文字数**で cap するため CRLF は行数分だけ実効文字数を増やし、上限保証が「出力行数 ≦ 約 200 行」という**書かれていない条件付き**に退化していた。変換を止めて `len(出力文字列)` ＝ ハーネスが数える文字数 を 3 OS で一致させる。`/CLAUDE.md` §9-3 の canonical 順序（stdin → stdout → stderr）は不変で、キーワード引数の追加のみ
...
- **dev-workflow C-2 に C-3省略宣言ブロック新設（E→C差し戻し時のレビュー差し戻し再承認のみ・HITL 専用）**: レビューフェーズ E で指摘が出て C へ差し戻される場合に限定して、差し戻し後の C-2 再承認時に 「修正計画が findings の直接反映のみで上流文書との差分がない」という客観条件を満たせば、C-3（design-critic 監査）を質問なく省略できる機構を追加。省略条件は 4 条件の AND: **(1) 直接反映のみ**（テスト追加・文言修正・レビュアー提示の具体案の移植のみ。複数 findings 一括解消のための方式変更・既存テスト構造の変更は不成立側）/ **(2) 帰属アンカー**（当該差し戻しの帰属判定記録に architect 帰属を含まず、かつ note に固定トークン `帰属根拠:明確` が含まれること）/ **(3) 上流文書との無差分**（requirements/architecture との矛盾なし・文書改訂不要）/ **(4) 新規導入なし**（新規機構・新規ファイル導入を伴わない）。暗黙スキップ禁止・判定不能・曖昧なら質問側に倒す fail-safe 設計。4 条件に達した C-2 再承認では、新規の 4 択 JSON を提示（「承認」「承認（監査は実施）」「否認・修正を依頼する」「否認・自分でファイルを編集する」）し、ユーザーの明示宣言により監査省略を確定する。
- **dev-workflow C-3 ステップ 0 新設（転記行の確認・HITL 専用）**: C-2 の C-3省略宣言ブロックで生成された転記行（`C-3省略宣言:` / `C-3監査要求:`）をセッションファイルで評価する新規ステップ。当日セッションファイル（`.claude/memory/sessions/{YYYYMMDD}.tmp`）の `現在地:` 行の直後に置かれた未消費転記行のうち、現行 plan タイムスタンプと一致するものを判定対象とする（位置限定・複数マッチ不成立）。**順 1**: `C-3監査要求:` があればステップ 2（design-critic 起動・ユーザーが C-2 で監査実施を選択済み）/ **順 2**: `C-3省略宣言:` があればフェーズ D へ進む（省略成立・ステップ 1〜5 実行なし）/ **順 3**: 転記行なし→従来入口（初回はステップ 1・層別ルーティング復帰はステップ 5）。転記行の消費状態を `(消費済):` へ Edit することで、同一セッション内での重複消費を防ぐ。日跨ぎで転記行が失われた場合は fail-safe で質問側に倒す。
- **plan-report の新規タイムスタンプ必須化（全経路）**: E→C 差し戻し・C-3 層別ルーティング・C-2 否認のいずれで plan-report を再生成する場合も、ピンポイント修正であっても既存 plan-report の Edit・同一タイムスタンプでの再承認は禁止し、必ず新規タイムスタンプで Write する。転記行と plan の 1:1 紐付け（ステップ 0 の ts 一致判定が古い plan と新しい plan を混同しない）と「最新 plan タイムスタンプ = 現行」の成立を守る書式規約。
- **E-1/E-2 帰属判定に帰属根拠トークン必須化**: tier-routing 結果記録（帰属判定）の `--note` に固定トークン `帰属根拠:明確` / `帰属根拠:要判断` を必ず含める規約を新設。迷いなく最上流起因 role を特定できた場合のみ `帰属根拠:明確`、迷った末に選んだ場合は `帰属根拠:要判断`（複数指摘の集約の場合は最上流判定に迷いが無いことが基準）。このトークンは C-2 の C-3省略宣言の帰属アンカー条件 (2) で直接参照され、省略の成否を左右する肯定表明。トークン不在・`帰属根拠:要判断`・判定記録が取得できない場合は省略不成立側に倒す。

### 変更

- **dev-workflow C-2 節を約 87 行拡張**: 新規タイムスタンプ規約・C-3省略宣言ブロック（4 条件 AND・省略条件の詳細判定手順・固定書式・3 段取得フォールバック・セッションファイルへの転記位置必須化・4 択/3 択 JSON の併記）・4 択新設 JSON の分岐条件がインライン追加される（E→C 差し戻し後のレビュー差し戻し C-2 再承認という発生頻度の低い分岐）。初回 C-2・条件不成立時は従来の 3 択 JSON を使用。
- **dev-workflow C-3 ステップ 1 の説明を改訂**: opt-in の確認に「省略は『質問の省略』であって監査の禁止ではない（ユーザーはいつでも監査を明示要求できる）」という意味合いを追記。
- **autonomous-mode SKILL.md のゲート対応表 C-3 行に注記追加**: 自律モードでは C-3 全体が本表に従う（design-critic を必ず起動・差し戻し後も上限 2 サイクルの別枠・転記行の生成・評価は行わない）。理由: 自律は人間の承認の目が無いため機械監査を厚く保つ。ステップ 0 の転記行確認は HITL 専用・自律モードでは実施しない注記も明記。
- **autonomous-mode SKILL.md に新規注記を追加**: 「dev-workflow C-3 のステップ 0（転記行確認）と転記行生成の C-2 ブロックは HITL 専用。自律モードでは C-3 全体が本表に従い、差し戻し後も design-critic を必ず起動する。転記行 2 種の生成・評価も行わない」。

### 後方互換

- **破壊的変更なし**。本リリースは `.claude/skills/dev-workflow/SKILL.md`（+88/-4）と `.claude/skills/autonomous-mode/SKILL.md`（+3/-1）の文書規約のみを追加・変更する。実装コード・hook・テストの変更は伴わない（confirm- タスク・design-review 6 サイクル・code-review・security-review で検証済み）。初回 C-3（C-2 承認直後に初めて到達）の挙動は変わらず（毎回 opt-in 質問を出す）。C-3省略は E→C 差し戻し後という限定シーンで発動し、既定の HITL・opt-in なし挙動を壊さない不変則（`.claude/CLAUDE.md` Approval Flow）を保持。
...
- **TOML エスケープが制御文字を素通しし、Codex アダプター生成物が不正な TOML になる問題を修正**: `adapters.py` の `_toml_escape()` / `_toml_multiline_escape()` が TOML 仕様上禁止された制御文字をエスケープせず、生成物（`.codex/config.toml` / `.codex/agents/*.toml`）が不正な TOML になる、または行構造を偽装されうる defect を修正した。エスケープ表を `str.translate()` 用の module 定数として構築し、C0 / DEL / C1 / U+2028 / U+2029 を封じる（TOML 必須分に加え、`str.splitlines()` が行境界とみなす 10 文字を対象とする）。複数行用の表は基本文字列用からの派生生成にし、手書きの二重管理で VT(U+000B) が脱落していた問題を解消した。あわせて `_escape_toml_quote_runs()` を 1 パス化し、2 パス構成が自身の出力した `\"` を生クォートと誤認して末尾クォート連 3 個以上で値を破壊していた問題を修正した。境界値テスト 49 件を追加（tomllib 往復検証・行境界 10 種・2 表の対称性・14 agent の不変条件・環境非依存スナップショット）
- 読み戻し側（`cli_doctor.py` の正規表現）は変更していない。生成側で行境界文字を封じれば行構造偽装は成立しないため

### 後方互換

- **破壊的変更なし**。修正は「制御文字を含む入力に対して不正な TOML を生成しなくなる」方向のみで、制御文字を含まない正常な入力の生成結果は不変。既存の Codex アダプター生成物は挙動が変わらない
