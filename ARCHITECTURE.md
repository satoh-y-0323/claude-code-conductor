...
| `PreToolUse` | `Write` | `report_path_guard.py` | レポート 9 prefix の書き先を検査し逸脱を block（封じ込め: `.claude/reports/` 直下の二層判定 / 上書き: タイムスタンプ形式の既存ファイル / 形式: strict-4 の非タイムスタンプ命名。`C3_REPORT_GUARD_DISABLE=1` で無効化） |
