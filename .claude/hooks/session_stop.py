...
設計判断:
- consolidate_memory.py は `--llm-only` 子プロセス起動のため独立スクリプトとして
  残す必要がある。本ファイルは run_sync() を関数として直接呼ぶ。
- stop.py の main() / consolidate_memory.py の _full_sync_main() は後方互換のため
  残す（手動実行や旧 settings.json からの呼び出し対応）。
"""
...
def _has_recent_sessions(sessions_dir: str, window_days: int = 7) -> bool:
...
def main() -> int:
    ...
    try:
        flag_path = os.path.join(_CLAUDE_DIR, "state", "llm_summary_agent_requested.flag")
        ...
        print(f"[session_stop:flag_control] failed: {e}", file=sys.stderr)
        return 0
