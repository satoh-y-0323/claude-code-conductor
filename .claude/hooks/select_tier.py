...
スコープ（ユーザー承認済み・tier-routing 自動適用フェーズ3で機械適用へ移行）:
- **やる**: 推奨 Tier を提示し、developer / wt_developer の Agent 起動時に
  PreToolUse hook（tier_autoapply.py）が `model:` へ機械適用する
  （updatedInput での自動書き換え・フェーズ3実装済み）。select_tier 自身は
...
- Agent 起動時に ``tier_autoapply.py`` が実適用 model を
  ``.claude/state/tier_autoapply.jsonl`` へ記録する（適用者=記録 SSOT）
- dev-workflow の各フェーズ承認ゲート（並列タスク単位を含む）で
  ``record_agent_outcome.py`` が applied-state（tier_autoapply.jsonl）を
  session_id 一致で読んで実適用 tier を機械解決し（LLM 申告を介さない帰属機械化）、
  ``agent_outcomes`` イベントログに記録する。tier_selection.json はフォールバック
  互換のため優先3 として温存する。
...
def _prompt_prefix_and_hash(prompt: str) -> tuple[str, str]:
    ...
    SR-K-003: 全文をマスクしてから 200 字へ切り詰めた（マスク→切り詰め順）。
    切り詰め→マスク順だと END タグが 200 字境界より後方にある PEM 等で
    マスクがすり抜け鍵本体が平文残存したため是正した。
...
def build_additional_context(
    ...
    *,
    ...
    return (
        ...
        f"（tier_autoapply）がこの推奨 Tier を model: に自動適用します"
