...
MVP スコープ（ユーザー承認済み・tier-routing 自動適用フェーズ2でソフト適用を追加）:
- **やる**: 推奨 Tier を提示し、developer / wt_developer の Agent 起動時に
  `model:` へ明示適用するよう親 Claude に指示する（ソフト適用・
  additionalContext 経由の行動指示であり機械強制ではない）。agent の
  `model:` フロントマター自体は変更しない
- **やらない**: PreToolUse hook 等による機械適用（updatedInput での自動書き換え・
  次フェーズ）、developer 以外の role への適用、フォールバック（Haiku 失敗→Sonnet 自動昇格）
...
def build_additional_context(
    ...
    *,
    ...
    return (
        ...
        f" 指示: developer / wt_developer を Agent ツールで起動する際は"
        f" model: {tier} を明示指定してこの推奨 Tier を適用すること"
        f"（学習データ収集中の期間も含め常に適用する。fork は model 上書き不可のため対象外）。"
