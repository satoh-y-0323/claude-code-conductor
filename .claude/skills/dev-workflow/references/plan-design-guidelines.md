...
| 両モード共通 | ルール 4・5・7・8・12・13・15・16・17・18・19・20 | R2・R3・R4・R6（planner_check hook 群は po_plan_version の値を読まないため sequential プランでも発火する） | id 一意・depends_on 参照先実在／空配列禁止／TDD 3 タスク分解／15 分粒度／R6／writes が空のタスク／レビュータスクの depends_on／ルール 15 方向検算／テスト凍結の実装責任／手順過多の契約化／一時改変の後始末方法／検知力検証の双方向／gitignored 検証パス |
...
16. **テスト前提の実装責任** — 計画が既存テストの凍結を前提とする場合（件数リテラル・逐語アンカー・特定テストケースの存在仮定など）、その仮定を変更する修正は **当該テストファイルを `writes` に含むタスク** で行うこと。修正担当を明示することにより、前提が破られたときの責任の所在を確定させる。

## タスク prompt の設計方針（ルール 17）

17. **契約と性質で指示し、手順で指定しない** — `tasks[].prompt` には契約・不変則・測るべき性質を書く。逐語的な手順指定は避けること。実施主体が判断を失い、手順の逐語に従うだけになると別の欠陥を生じやすい。実測: C-3 監査で High 5 件中 3 件が親による逐語手順の誤りが原因だった（2026-07-23）。

## 一時改変の後始末指定（ルール 18）

18. **cleanup 手段を明示・禁止経路を宣言する** — 検証のための一時改変（テストの一時ハード・mock 削除・debug logging など）を指示する場合、**後始末の手段を逐語で指定し、 `git checkout` / `git restore` / `git reset` を明示的に禁止すること**。これらは復元不能な失敗を招く（実績: 2026-08-01 に実装 5 時間分が全損）。代わりに「元のコードへ戻す」「テストスケルトンを削除」など、失敗時に検出可能な方法を指示すること。

## 検知力検証の双方向テスト（ルール 19）

19. **do-nothing スタブの両方向赤緑反転** — 検証系タスクの contract に「検知力を実証する」を含める場合、do-nothing スタブ **2 種**（always-False / always-True）を用いて赤緑が完全に入れ替わることを示すこと。単一スタブでは正の双子が赤にならない原理的限界を越えられない。適用困難な対象では、代替手段として「両方向反転」の実現可能性を明記する。

## gitignored 成果物の検証経路（ルール 20）

20. **直接 Read・状態外の指示分離** — gitignored ファイル（`.claude/reports/` 等）の検証は `git status --short` ではなく **直接 Read で行うこと**。git 管理下で成果物を検証する必要があれば、明示的に commit → 確認 → cleanup の流れを指示する。worktree で起動される worker（developer / tester）の prompt には gitignored 文書の Read を指示しないこと（届く経路が無い。親か plan-report 経由で指す）。reviewer 向けの prompt には「当該ファイルは gitignored である」と明記し、reviewer に直接 Read させることを指示する。
...
- [ ] ルール 16: テスト凍結仮定を変更する修正は当該テストファイルを writes に含む task で行うか明示されているか
- [ ] ルール 17: tasks[].prompt は逐語手順でなく契約・不変則・測るべき性質で書かれているか
- [ ] ルール 18: 一時改変指示は後始末手段を逐語で指定し、git checkout/restore/reset を明示的に禁止しているか
- [ ] ルール 19: 検証系 task の contract に do-nothing スタブ 2 種の赤緑反転実証が含まれているか（適用困難なら代替宣言）
- [ ] ルール 20: gitignored 成果物検証は直接 Read 指示で行い、worker prompt には Read を指示していないか
