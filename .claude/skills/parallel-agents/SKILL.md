...
- `wt_developer`→`developer` の記録は **`--tier {当該 wt_developer 起動時に hook（tier_autoapply）が model: へ自動適用した推奨 Tier}` を明示する**（tier_selection.json の worktree 自己解決には依存せず親が申告する）。この値の SSOT は `tier_selection.json.tier`（kickoff で 1 度確定・以降 wave をまたいで安定）で、2-C で hook が `model:` へ自動適用したのと同一値を渡す。並列 wave 内の複数 wt_developer は同一の推奨 Tier を共有する（単一 tier_selection.json・DC-AS-003 で許容済み）。逸脱時（明示指定した `model:` が推奨 Tier と異なる）はその実際に適用された tier を `--tier` に渡す（並列経路では常に実適用値を申告するため、逸脱の有無に依らず `--tier` を付ける運用に一本化される）。**hook は worktree 側で実適用 model を `.claude/state/tier_autoapply.jsonl` に記録するが、worktree の state 分離により親 main の record からは session_id 一致で読めないことがあるため、並列経路では初回は `--tier` 明示を維持する（applied-state 経由の自己解決への一本化は T8 で対応）。**
...
python .claude/skills/dev-workflow/scripts/record_agent_outcome.py \
  ...
  --tier {当該 wt_developer 起動時に hook が model: へ自動適用した推奨 Tier（tier_selection.json.tier と同値）} \
  --task {task_id}
...
**`--tier` の付与ルール（ADR-AS-4・2-E と同一）**: `wt_developer`→`developer` は **`--tier {当該 wt_developer 起動時に hook（tier_autoapply）が model: へ自動適用した推奨 Tier（tier_selection.json.tier と同値）}` を明示**する（worktree 自己解決に依存しない・並列 wave 内の複数 wt_developer は同一 tier を共有〈DC-AS-003〉・逸脱時は実際に適用された tier を渡す）。**hook は実適用 model を `.claude/state/tier_autoapply.jsonl` に記録するが、worktree の state 分離により親 main の record が読めないことがあるため並列経路では初回 `--tier` 明示を維持する（T8 で一本化）。** `wt_tester`→`tester` は **`--tier` を付けない**（frontmatter 解決のまま不変・機械適用対象外）。
