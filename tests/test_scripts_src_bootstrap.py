...
1. `scripts/*.py` のうち `c3` を import しているファイルを動的に列挙する
   （ハードコードしない。対象ファイルが増えても黙って検査を免れない）
...
壊れる**（現行の対象ファイルは stdlib のみと実測済みだが、その前提に検査の正しさを
...
def _iter_scripts_importing_c3() -> list[Path]:
    ...
    新たなファイルが追加され `c3` を import しても、リストへの追記漏れで
...
class TestResolveC3FileViaSubprocessDetectsViolations:
    ...
    ことの合成スクリプトでの裏取り。実運用ファイル（現状は全て正しい）だけでは
