...
`rel in REINSTALL_TRIGGERS` の比較対象は relpath 後の ASCII 定数集合なので、
挙動を変えうるのは **relpath 前段の入力（cwd と abs_target）の表現差** だけである。
...
観測される現行の欠陥は 2 つあり、いずれも「無警告（fail-open）」として現れる:
    ...
    欠陥 2: NFC 正規化欠落 — relpath 前に正規化しないため表現差で rel が
            `../repo-<NFD>/src/c3/__init__.py` のような別パスになる
...
def _init_py(root: Path) -> str:
    return str(root / "src" / "c3" / "__init__.py")
...
def test_nfd_absolute_path_warns_with_utf8_payload(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0
    assert "[PipReinstallReminder]" in result.stderr, (
        "NFD 形の絶対パスでも NFC 正規化後は src/c3/__init__.py と一致し警告されるべきだが "
...
def test_nfc_absolute_path_with_escaped_payload_still_warns(tmp_path: Path) -> None:
    ...
    result = _run_hook(_payload("Write", _init_py(nfc_root)), cwd=str(nfc_root))
...
def test_relative_path_in_non_ascii_repo_still_warns(tmp_path: Path) -> None:
    ...
    result = _run_hook(_payload("Write", "src/c3/__init__.py"), cwd=str(nfc_root))
...
def test_unrelated_file_under_nfd_root_stays_silent(tmp_path: Path) -> None:
    """[S3M1・回帰] NFD ルート配下でも REINSTALL_TRIGGERS 外なら無警告のまま。
    ...
    assert result.returncode == 0
    assert result.stderr == "", (
        f"REINSTALL_TRIGGERS 外のファイルで警告が出た: {ascii(result.stderr)}"
    )
