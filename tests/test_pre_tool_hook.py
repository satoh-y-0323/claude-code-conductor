...
# ---------------------------------------------------------------------------
# [Sec High-2] cd bypass detection — correct behavior tests
# ---------------------------------------------------------------------------


class TestCdBypassDetection:
    """Tests for cd bypass detection logic."""
    ...
    # --- [Sec High-2] BUG TESTS (expected to FAIL on current implementation) ---

    def test_subshell_cd_is_blocked(self):
        """BUG [Sec High-2]: '$(cd /tmp)' via subshell must be blocked.

        Current pattern r'(?:^|[;&|])\\s*cd(?:\\s|$)' does not match
        subshell syntax '$(' preceding 'cd'.

        This test FAILS on the unfixed implementation.
        """
        result = _run_hook("$(cd /tmp)")
        assert _is_blocked(result), (
            "[Sec High-2 BUG] '$(cd /tmp)' subshell bypass was not detected.\n"
            f"exit={result.returncode}, stderr={result.stderr}"
        )

    def test_backtick_subshell_cd_is_blocked(self):
        """BUG [Sec High-2]: '`cd /tmp`' via backtick subshell must be blocked.

        Current pattern does not include backtick '`' as a separator.

        This test FAILS on the unfixed implementation.
        """
        result = _run_hook("`cd /tmp`")
        assert _is_blocked(result), (
            "[Sec High-2 BUG] '`cd /tmp`' backtick bypass was not detected.\n"
            f"exit={result.returncode}, stderr={result.stderr}"
        )

    def test_eval_cd_is_blocked(self):
        """BUG [Sec High-2]: \"eval 'cd /tmp'\" must be blocked.

        Current pattern does not handle eval-based cd invocations.

        This test FAILS on the unfixed implementation.
        """
        result = _run_hook("eval 'cd /tmp'")
        assert _is_blocked(result), (
            "[Sec High-2 BUG] \"eval 'cd /tmp'\" bypass was not detected.\n"
            f"exit={result.returncode}, stderr={result.stderr}"
        )

    def test_newline_separated_cd_is_blocked(self):
        """BUG [Sec High-2]: multiline command with cd on second line must be blocked.

        Current pattern includes newline '\\n' in separators? Let's verify.
        'echo hello\\ncd /tmp' — newline as separator is not in the current pattern.
        """
        result = _run_hook("echo hello\ncd /tmp")
        assert _is_blocked(result), (
            "[Sec High-2 BUG] Newline-separated 'cd /tmp' was not detected.\n"
            f"exit={result.returncode}, stderr={result.stderr}"
        )
